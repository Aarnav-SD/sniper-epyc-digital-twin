...
namespace intrabarrier_analysis{
...
enum class PredictResultType:uint8_t{
    ...
    class Feature {
        public:
            ...
            Feature( uint64_t fs_, const std::vector<uint64_t> & insnums, mtng::RecordType record_type_in ):
            fs(fs_), record_type(record_type_in),solved(true) //use for detailed mode
            {
...
#define queue_size 1
        ...
        std::string toJson() {
            std::stringstream out ;
            //////////feature stack
            time_stamp_end = getSystemTime();
            double walltime = time_stamp_end - time_stamp_begin;
            out << "{";
...
//                }
        ...
        Feature<dvfs_enable> get_ipc() {
            ...
            prior_fs = fs;
            return Feature<dvfs_enable>(  interval,  ins_num_interval_vec,last_record_type );
        }
